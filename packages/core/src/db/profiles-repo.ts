...
import type { SearchProfile, PlatformName } from '../types'
...
export class ProfilesRepo {
  ...
  getById(id: string): SearchProfile | null {
    const db = getDatabase()
    const row = db.prepare('SELECT * FROM search_profiles WHERE id = ?').get(id) as
      | ProfileRow
      | undefined
  ...
  update(
    ...
    params.push(id)
    db.prepare(`UPDATE search_profiles SET ${sets.join(', ')} WHERE id = ?`).run(...params)
  }
  ...
  toggleEnabled(id: string, enabled: boolean): void {
    const db = getDatabase()
    db.prepare('UPDATE search_profiles SET enabled = ?, updated_at = ? WHERE id = ?').run(
      enabled ? 1 : 0,
