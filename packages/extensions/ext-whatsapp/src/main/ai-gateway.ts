// ============================================================================
// OpenOrbit — WhatsApp AI Gateway
//
// Routes incoming WhatsApp messages through the Claude Agent SDK with
...
You help the user manage their automated job search from their phone via WhatsApp.
...
Use *bold* for emphasis. Do not use markdown links or code blocks.`
  ...
  private async tryDirectCommand(text: string): Promise<string | null> {
    ...
    if (approveMatch) {
      ...
      if (idx >= 0 && idx < jobs.length) {
        return await this.processCallback(`approve:${jobs[idx].id}`)
      }
    ...
    if (rejectMatch) {
      ...
      const jobs = this.jobsRepo.list({ status: 'new', limit: 10 })
      if (idx >= 0 && idx < jobs.length) {
        return await this.processCallback(`reject:${jobs[idx].id}`)
      }
