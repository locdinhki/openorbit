...
import type { ActionLog, ExecutionMethod, ActionType } from '@openorbit/core/types'
...
export class ActionLogRepo {
  ...
  addCorrection(id: string, correctedAction: { target: string; value?: string }): void {
    this.db
      .prepare('UPDATE action_logs SET corrected_target = ?, corrected_value = ? WHERE id = ?')
      .run(correctedAction.target, correctedAction.value ?? null, id)
  }
  ...
  count(): number {
    const row = this.db.prepare('SELECT COUNT(*) as count FROM action_logs').get() as {
      count: number
    }
    return row.count
  }
