...
describe('formatJobList', () => {
  ...
  it('uses plain text (no markdown)', () => {
    const jobs = [makeJob()]
    const result = formatJobList(jobs, 'Jobs')
    // Should not contain Telegram-style markdown bold markers
...
describe('formatJobDetail', () => {
  ...
  it('uses plain text (no markdown)', () => {
    const result = formatJobDetail(makeJob())
    expect(result).not.toMatch(/\*[^*]+\*/)
  })
})
...
describe('formatProfileList', () => {
  ...
  it('formats profiles with emoji indicators', () => {
    const profiles = [
