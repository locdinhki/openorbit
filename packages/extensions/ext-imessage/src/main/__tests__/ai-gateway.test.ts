...
// Mock ext-jobs repos using class syntax for proper constructor support
...
describe('AIGateway', () => {
  ...
  describe('direct commands', () => {
    ...
    it('uses plain text (no markdown)', async () => {
      const result = await gateway.handleMessage('/help')
      // Should not contain Telegram-style *bold* markers
  ...
  describe('callback processing', () => {
    ...
    it('uses plain text in callback responses', async () => {
      const result = await gateway.processCallback('approve:job-1')
      // Should not contain *bold* markdown
      expect(result).not.toMatch(/\*[^*]+\*/)
    })
  })
